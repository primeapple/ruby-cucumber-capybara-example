...
# https://github.com/MacCracken/watir-performance/blob/master/lib/watir-performance.rb
...
module Capybara
  ...
  PERFORMS = 'JSON.stringify((window.performance || window.webkitPerformance || '\
             'window.mozPerformance || window.msPerformance).toJSON());'.freeze
  ...
  class Session
    def performance
      ...
      #          script = 'JSON.stringify(window.performance.toJSON());'
      #          cmd = evaluate_script(script)
      #          ::Object::JSON.parse(cmd)
      #        else
      #          evaluate_script(PERFORMS)
      #        end
      data = evaluate_script(PERFORMS)
